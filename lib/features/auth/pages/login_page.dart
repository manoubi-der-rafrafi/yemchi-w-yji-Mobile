  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!ok) {
        setState(
          () =>
              _errorMsg = auth.error.value ?? 'Email ou mot de passe incorrect',
        );
        HapticFeedback.heavyImpact();
        return;
      }
      _goToHome(auth.currentUser.value);
    } finally {
      if (mounted) setState(() => _loading = false);
    }
  }

  void _goToHome(Utilisateur? user) {
    final route =
        switch (user?.role) {
          Role.transporteur => '/home_coursier',
          Role.client => '/home_client',
          _ => '/login',
        };
